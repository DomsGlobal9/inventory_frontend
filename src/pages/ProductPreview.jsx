...
export default function ProductPreview() {
    ...
    const buildVariants = (productId) => {
      ...
      productData.selectedColors.forEach(colorCode => {
        ...
        productData.selectedSizes.forEach(size => {
          ...
          const sku = `${payload.productCode}-${safeName}-${size}`;
          variants.push({
            sku,
            size,
    ...
    if (productData.id) {
      ...
      createMutation.mutate(payload, {
        onSuccess: async (res) => {
          ...
          const variants = buildVariants(newProductId);

          if (newProductId && variants.length > 0) {
            try {
              await bulkCreateVariants(newProductId, variants);
            } catch (err) {
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', gap: '48px', paddingTop: '32px' }}>
      
      {/* Left: Product Images */}
      <div style={{ flex: '1', maxWidth: '400px' }}>
        <div className="glass-panel" style={{ height: '500px', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
          {(productData.imageUrls || []).slice(1, 4).map((url, i) => (
            <div key={i} className="glass-panel" style={{ aspectRatio: '3/4', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ flex: '1', maxWidth: '300px' }}>
        <div className="glass-panel" style={{ padding: '24px', position: 'sticky', top: '24px' }}>
          ...
          <div style={{ padding: '16px', backgroundColor: 'var(--bg-input)', border: '1px solid var(--border-light)', borderRadius: '4px', marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '32px' }}>
            {[
              { label: 'Product Information', done: productData.title && productData.category },
              { label: 'Sizes Added', done: productData.selectedSizes.length > 0 },
              { label: 'Colors Added', done: productData.selectedColors.length > 0 },
              { label: 'Stock Units Filled', done: totalUnits > 0 },
              { label: 'Base Price Set', done: productData.price > 0 }
            ].map((item, idx) => (
